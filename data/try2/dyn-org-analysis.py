...
def loop_per_row(df_adj, google_trends_data):
    ...
    google_trends_data_short=google_trends_data[:100]
    max_val = 1
    df_new=df_adj.copy()
    ...
    for i in range(0,df_new.shape[0]):
        ...
        for h in range(0, len(scaled_short_list)):
            ...
            error_addition=abs(scaled_short_list[h]-google_trends_data_short[h])
        ...
        if thresh >= cutoff:
            ...
            print('nope',i)
        corr_coef=pearsonr(google_trends_data_short,scaled_short_list)
        corr_val=corr_coef[0]
        df_new['corr_coef'].iloc[i] = corr_val
        RMSE = (mean_squared_error(google_trends_data_short, scaled_short_list, squared=True))
    ...
    plt.plot(time_list, google_trends_data_short, label= 'google trends')
    plt.legend()
    plt.title("Min RMSE Dynamic Organizing Sweep")
    plt.show()
    ...
    plt.plot(time_list, google_trends_data_short, label='google trends')
    plt.legend()
    plt.title("Max CC Dynamic Organizing Sweep")
    plt.show()
    ...
    plt.plot(time_list, google_trends_data_short, label= 'google trends')
    plt.legend()
    plt.title("Max RMSE Dynamic Organizing Sweep")
    plt.show()
