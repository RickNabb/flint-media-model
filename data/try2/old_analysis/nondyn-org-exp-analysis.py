...
def createdataframe(dataset):
    ...
    df.columns = ['run', 'n', 'spread-type', 'simple-spread-chance', 'graph-type', 'ba-m', 'cit-media-influence', 'cit-cit-influence', 'organizing-capacity',
                  'organizing-strategy', 'data']
    print(df)
    return df
...
def loop_per_row(df_adj, google_trends_data):
    ...
    for i in range(0,df_new.shape[0]):
        ...
        df_new['max_height'].iloc[i]=max(short_list)
        ...
        print('error', error)
    df_new.to_csv("nondyn-org-exp-eval.csv")
...
main('nondynamic-organizing-exp-results.csv')
